...
Hệ thống SHALL có component `FieldMap.vue` dùng `<svg>` full-width, `preserveAspectRatio="none"`, cao hơn MapCanvas Home (~220px), vẽ field rectangles (`fill-primary-200 stroke-primary-300`), pipe dots (`fill-blue-400`), và label text A/B/C (`text-neutral-900`) ở góc trên-trái mỗi rectangle.
...
### Requirement: FieldListRow component (không có cm)
Hệ thống SHALL có component `FieldListRow.vue` hiển thị dạng card (`bg-white border border-neutral-200 rounded-2xl p-4`): tên field (`text-neutral-900 text-base font-medium`), sub info ha·pipes (`text-neutral-500 text-xs`), và chevron `›` (`text-neutral-300`). KHÔNG hiển thị water level cm.
...
### Requirement: Danh sách fields với gap
Các FieldListRow SHALL được render trong container `flex flex-col gap-3`, mỗi row là 1 card riêng biệt.
...
### Requirement: FAB thêm field mới
Fields Top page SHALL có FAB (Floating Action Button) dùng `<UButton icon="i-lucide-plus" color="primary">` với class `fixed bottom-24 right-5 rounded-full shadow-lg z-40 size-14`, navigate đến `/fields/new` khi click.
