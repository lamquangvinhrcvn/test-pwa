...
Page `app/pages/fields/new.vue` SHALL sử dụng `layout: 'default'` và render `<AppHeader>` với `#back` slot, `#title` = "New Field", `#action` = "Save" text button (`<UButton variant="ghost" class="text-white">`). Save button disabled nếu chưa đủ điều kiện.
...
Hệ thống SHALL có component `DrawableMap.vue` dùng `<svg>` full-width, `preserveAspectRatio="none"`, cao ~300px với background `fill-primary-100`. User tap để thêm điểm, khi có ≥3 điểm thì vẽ `<polygon>` preview (`fill-primary-200/50 stroke-primary-300`). Mỗi điểm có circle handle màu trắng viền primary-300, draggable (kéo bằng touch/mouse). Có nút "Reset" để xóa tất cả điểm.
...
Hệ thống SHALL có utility `utils/geometry.ts` export hàm `polygonArea(pts: Point[]): number` dùng Shoelace formula tính diện tích polygon (đơn vị px²), và hàm `pixelToHa(pxArea: number): number` chuyển đổi pixel² sang hectares qua hằng số `PX2_PER_HA`.
...
### Requirement: Form Field Name và Field Note
New Field page SHALL có `<UInput>` cho Field Name (required, placeholder "e.g. Field D") và Field Note (optional, placeholder "Optional note..."), cả hai dùng `color="primary" size="lg"`.
...
New Field page SHALL hiển thị diện tích tự động tính từ DrawableMap polygon, format `X.XX ha`, trong 1 read-only input hoặc text display với label "Area (auto-calculated)".
...
### Requirement: Save Field button và validation
New Field page SHALL có `<UButton color="primary" variant="solid" size="lg" block>` với text "Save Field". Button disabled nếu `fieldName` rỗng hoặc `points.length < 3`. Khi save thành công, navigate về `/fields`.

#### Scenario: Save button disabled khi thiếu dữ liệu
- **WHEN** user chưa nhập tên field hoặc chưa vẽ đủ 3 điểm
- **THEN** Save button ở trạng thái disabled

#### Scenario: Save thành công
- **WHEN** user đã nhập tên và vẽ polygon hợp lệ, tap "Save Field"
- **THEN** app navigate về `/fields` (mock save)

### Requirement: Mock data và không hardcode hex
