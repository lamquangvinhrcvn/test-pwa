...
Bottom sheet / modal để nhập water level (cm) cho một pipe. Dùng UDrawer, có date picker, stepper ±, và save/cancel.
...
### Requirement: Date input có thể chọn ngày
WaterLevelDialog SHALL có date section với icon `i-lucide-calendar` và `<input type="date">` cho phép user chọn ngày. Mặc định là ngày hôm nay.

#### Scenario: Date input hiển thị hôm nay
- **WHEN** dialog mở
- **THEN** date input hiển thị ngày hôm nay

### Requirement: WaterLevelStepper custom component
Hệ thống SHALL có component `WaterLevelStepper.vue` là custom stepper: container `bg-blue-100 rounded-xl p-3`, layout `[−] [value] cm [+]`. Nút `−` và `+` dùng `<UButton variant="outline" color="neutral">` với border tròn `rounded-full border-blue-300`. Giá trị hiển thị `text-blue-800 text-3xl font-bold`. Đơn vị "cm" hiển thị `text-blue-800 text-sm`. Prop `modelValue` (number), emit `update:modelValue`.
...
WaterLevelDialog SHALL có 2 nút ở footer: Cancel (`<UButton variant="ghost" color="neutral">`) và Save (`<UButton variant="solid" class="!bg-primary-500">`). Save disabled khi value không thay đổi hoặc null. Cancel đóng dialog không lưu. Save emit event `save` với `{ pipeId, date, level }` rồi đóng dialog.
